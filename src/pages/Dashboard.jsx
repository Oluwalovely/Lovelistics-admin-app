...
import {
    ...
    Star, Trophy, Medal, Bell, User, Car, Bike, Truck, ChevronRight
} from 'lucide-react';

const C = {
    navy:'#0d1f4f', navyL:'#162660', orange:'#e8610a', teal:'#0d9488',
    amber:'#d97706', red:'#dc2626', green:'#16a34a', purple:'#7c3aed',
    blue:'#2563eb', bg:'#f1f4f9', white:'#ffffff', border:'#e8edf7',
...
const ordersPerDay = orders => {
    ...
    return Object.entries(m).sort((a,b)=>new Date(a[0])-new Date(b[0])).slice(-7).map(([date,count])=>({date,count}));
};
...
const Dashboard = () => {
    ...
    useEffect(() => {
        fetchData();
        const poll = setInterval(async()=>{ try{ const dR=await getAllDrivers(); setDrivers(dR.data.data); }catch{} },15000);
    ...
    const filtered = useMemo(()=>{
        ...
        if (q) list=list.filter(o=>o.trackingNumber?.toLowerCase().includes(q)||o.customer?.fullName?.toLowerCase().includes(q)||o.driver?.fullName?.toLowerCase().includes(q));
        list.sort((a,b)=>{
        ...
        <div style={{ minHeight:'100vh',background:C.bg,fontFamily:"'Sora',system-ui,sans-serif",overflowX:'hidden',width:'100%',boxSizing:'border-box' }}>
            <style>{`
                ...
                *,*::before,*::after{box-sizing:border-box;}
                ...
                .kpi-grid>*{animation:fadeInUp 0.45s ease both;}
                .kpi-grid>*:nth-child(1){animation-delay:0.05s;}
                .kpi-grid>*:nth-child(2){animation-delay:0.10s;}
                .kpi-grid>*:nth-child(3){animation-delay:0.15s;}
                .kpi-grid>*:nth-child(4){animation-delay:0.20s;}
                .kpi-grid>*:nth-child(5){animation-delay:0.25s;}
                .kpi-grid>*:nth-child(6){animation-delay:0.30s;}
                .orders-panel {animation:fadeInUp 0.45s ease 0.15s both;}
                .driver-panel {animation:fadeInUp 0.45s ease 0.25s both;}
                .insights-grid>*:nth-child(1){animation:fadeInUp 0.45s ease 0.20s both;}
                .insights-grid>*:nth-child(2){animation:fadeInUp 0.45s ease 0.25s both;}
                .insights-grid>*:nth-child(3){animation:fadeInUp 0.45s ease 0.30s both;}
                .insights-grid>*:nth-child(4){animation:fadeInUp 0.45s ease 0.35s both;}
                .topcust-grid>*:nth-child(1){animation:scaleIn 0.4s ease 0.25s both;}
            ...
            <div className="dash-wrap" style={{ paddingTop:'1.5rem' }}>
                ...
                <div className="ops-grid">
                    <div className="orders-panel" style={{ background:C.white,borderRadius:14,border:`1px solid ${C.border}`,overflow:'hidden',...shadow }}>
                            ...
                            <div className="filter-bar">
                                {['all','pending','active','in-transit','delivered','delayed','cancelled'].map(f=>(
                                    <button key={f} onClick={()=>{ setStatusFilter(f); setPage(1); }}
                                        style={{ padding:'0.35rem 0.85rem',borderRadius:8,fontSize:'0.74rem',fontWeight:600,cursor:'pointer',textTransform:'capitalize',flexShrink:0,transition:'all 0.15s',border:statusFilter===f?'none':`1px solid ${C.border}`,background:statusFilter===f?C.navy:C.white,color:statusFilter===f?'#fff':C.muted,fontFamily:'inherit' }}>
                        ...
                        <div className="table-wrap">
                            <table style={{ width:'100%',borderCollapse:'collapse' }}>
                                ...
                                <tbody>
                                    {paginated.length===0
                                        ? <tr><td colSpan={8} style={{ textAlign:'center',padding:'3rem',color:C.faint }}><Package size={32} style={{ opacity:0.25,display:'block',margin:'0 auto 0.5rem' }}/>No orders found</td></tr>
                                        : paginated.map(order=>{
                                            const hl=rowHighlight(order);
                                            return (
                                                <tr key={order._id} className="tr-hover" style={{ borderLeft:hl.left }}>
                                                    <td style={{ padding:'0.85rem 1rem',fontSize:'0.78rem',fontWeight:700,color:C.navy,whiteSpace:'nowrap',borderBottom:'1px solid #f8fafc',background:hl.bg }}>
                                                    ...
                                                    <td style={{ padding:'0.85rem 1rem',fontSize:'0.8rem',color:'#334155',whiteSpace:'nowrap',borderBottom:'1px solid #f8fafc',background:hl.bg }}>
                                                    ...
                                                    <td style={{ padding:'0.85rem 1rem',fontSize:'0.78rem',color:C.muted,maxWidth:130,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap',borderBottom:'1px solid #f8fafc',background:hl.bg }}>
                                                    ...
                                                    <td style={{ padding:'0.85rem 1rem',fontSize:'0.8rem',color:order.driver?'#334155':C.amber,fontWeight:order.driver?500:700,whiteSpace:'nowrap',borderBottom:'1px solid #f8fafc',background:hl.bg }}>{order.driver?.fullName||'Unassigned'}</td>
                                                    <td style={{ padding:'0.85rem 1rem',borderBottom:'1px solid #f8fafc',background:hl.bg }}><Badge status={order.status}/></td>
                                                    <td style={{ padding:'0.85rem 1rem',fontSize:'0.8rem',fontWeight:700,color:C.navy,whiteSpace:'nowrap',borderBottom:'1px solid #f8fafc',background:hl.bg }}>₦{order.price?.toLocaleString()||'—'}</td>
                                                    <td style={{ padding:'0.85rem 1rem',borderBottom:'1px solid #f8fafc',background:hl.bg }}>
                                    ...
                                    return (
                                        <div key={order._id} style={{ background:hl.bg,borderLeft:hl.left,borderRadius:12,padding:'1rem',marginBottom:'0.6rem',border:`1px solid ${C.border}` }}>
                                            ...
                                            <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center' }}>
                                                <div><p style={{ margin:0,fontSize:'0.68rem',color:C.faint }}>Driver</p><p style={{ margin:0,fontSize:'0.78rem',fontWeight:600,color:order.driver?C.navy:C.amber }}>{order.driver?.fullName||'Unassigned'}</p></div>
                                                <p style={{ margin:0,fontWeight:800,color:C.navy }}>₦{order.price?.toLocaleString()||'—'}</p>
                                                <Link to={`/orders/${order._id}`} style={{ display:'inline-flex',alignItems:'center',gap:3,background:C.navy,color:'#fff',padding:'5px 12px',borderRadius:7,fontSize:'0.74rem',fontWeight:600,textDecoration:'none' }}>Manage <ArrowRight size={11}/></Link>
                                            </div>
                                        </div>
                                    );
                                })}
                            ...
                            <div style={{ padding:'0.85rem 1.25rem',borderTop:`1px solid ${C.border}`,display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:'0.5rem' }}>
                                ...
                                <div style={{ display:'flex',gap:'0.3rem' }}>
                                    {Array.from({length:totalPages},(_,i)=>i+1).map(p=>(
                                        <button key={p} onClick={()=>setPage(p)} style={{ width:30,height:30,borderRadius:7,border:p===page?'none':`1px solid ${C.border}`,background:p===page?C.navy:C.white,color:p===page?'#fff':C.muted,fontSize:'0.78rem',fontWeight:600,cursor:'pointer',fontFamily:'inherit' }}>{p}</button>
                                    ))}
                                </div>
                            </div>
                        )}
                    ...
                    <div className="driver-panel" style={{ background:C.white,borderRadius:14,border:`1px solid ${C.border}`,padding:'1.25rem',...shadow }}>
                        <div style={{ display:'flex',justifyContent:'space-between',alignItems:'center',marginBottom:'1rem' }}>
                            ...
                            <div style={{ width:36,height:36,borderRadius:10,background:`${C.teal}15`,display:'flex',alignItems:'center',justifyContent:'center',color:C.teal }}><MapPin size={17}/></div>
                        </div>
                                    ...
                                    return (
                                        <div key={d._id} style={{ padding:'0.75rem',borderRadius:12,background:'#f8fafc',border:`1px solid ${C.border}`,marginBottom:'0.5rem',transition:'background 0.15s' }}
                                            ...
                                            <div style={{ display:'flex',alignItems:'center',gap:'0.6rem',marginBottom:'0.45rem' }}>
                                                <div style={{ width:36,height:36,borderRadius:'50%',background:C.navy,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',fontWeight:700,fontSize:'0.85rem',flexShrink:0 }}>{d.fullName?.charAt(0).toUpperCase()}</div>
                                                <div style={{ flex:1,minWidth:0 }}>
                                                    ...
                                                    <p style={{ margin:0,fontSize:'0.68rem',color:C.faint }}>{hasLoc?`${d.currentLocation.lat.toFixed(4)}° N, ${d.currentLocation.lng.toFixed(4)}° E`:'Location unavailable'}</p>
                                                </div>
                                            ...
                                            <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',paddingTop:'0.4rem',borderTop:`1px solid ${C.border}`,gap:'0.5rem' }}>
                                                <span style={{ fontSize:'0.7rem',color:C.muted,display:'flex',alignItems:'center',gap:4,minWidth:0,overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap' }}>
                                                    {d.vehicleType
                                                        ? <span style={{ display:'flex',alignItems:'center',gap:4,minWidth:0 }}>{getVehicleIcon(d.vehicleType)}<span style={{ textTransform:'capitalize',whiteSpace:'nowrap' }}>{d.vehicleType}</span>{d.vehiclePlate?<span style={{ whiteSpace:'nowrap' }}>· {d.vehiclePlate}</span>:''}</span>
                                                        : <span style={{ color:C.faint }}>No vehicle info</span>}
                                                </span>
                                                <span style={{ fontSize:'0.7rem',fontWeight:700,color:C.navy,background:'#e2e8f0',padding:'2px 7px',borderRadius:999,whiteSpace:'nowrap',flexShrink:0 }}>{d.totalDeliveries} {d.totalDeliveries===1?'delivery':'deliveries'}</span>
                                            </div>
                                        </div>
                                    );
                                })}
                    ...
                    <div style={{ background:C.white,borderRadius:14,padding:'1.25rem',border:`1px solid ${C.border}`,...shadow }}>
                        ...
                        <ResponsiveContainer width="100%" height={90}>
                            <BarChart data={chartData} margin={{top:0,right:0,left:-28,bottom:0}}>
                                ...
                                <XAxis dataKey="date" tick={{fontSize:10,fill:C.faint,fontFamily:'Sora'}} axisLine={false} tickLine={false}/>
                                <YAxis tick={{fontSize:10,fill:C.faint,fontFamily:'Sora'}} axisLine={false} tickLine={false} allowDecimals={false}/>
                                <Tooltip contentStyle={{borderRadius:9,border:'none',boxShadow:'0 4px 16px rgba(0,0,0,0.1)',fontSize:'0.78rem',fontFamily:'Sora'}} cursor={{fill:'#f1f5f9'}}/>
                                    ...
                                    <div key={c.id} style={{ background:C.white,borderRadius:14,padding:'1.4rem',border:`1px solid ${C.border}`,borderTop:`3px solid ${rankColor}`,...shadow,position:'relative',overflow:'hidden',transition:'transform 0.18s, box-shadow 0.18s',cursor:'default' }}
                                        ...
                                        <div style={{ position:'relative' }}>
                                            <div style={{ display:'flex',alignItems:'center',gap:'0.75rem',marginBottom:'1rem' }}>
                                                <div style={{ width:46,height:46,borderRadius:'50%',background:i===0?'#fef3c7':i===1?'#f1f5f9':'#fef2e8',display:'flex',alignItems:'center',justifyContent:'center',fontWeight:800,fontSize:'1.1rem',color:i===0?'#92400e':i===1?'#475569':'#92400e',flexShrink:0 }}>{c.name?.charAt(0).toUpperCase()}</div>
