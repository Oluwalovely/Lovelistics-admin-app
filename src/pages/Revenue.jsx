...
/* ── Driver Row — card on mobile, grid on desktop ── */
const DriverRow = ({ d, i, share, isMobile }) => {
    const medals = [C.orange, C.blue, C.purple];
    if (isMobile) {
        return (
            <div className="driver-row" style={{ padding: '0.9rem 1rem', borderBottom: `1px solid ${C.border}`, background: C.white, transition: 'background 0.12s' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem' }}>
                    <div style={{ width: 26, height: 26, borderRadius: 8, background: i < 3 ? `${medals[i]}15` : '#f1f4f9', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        {i < 3 ? <Award size={13} color={medals[i]} /> : <span style={{ fontSize: '0.7rem', fontWeight: 700, color: C.faint }}>{i + 1}</span>}
                    </div>
                    <div style={{ width: 34, height: 34, borderRadius: '50%', background: C.navy, display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff', fontWeight: 800, fontSize: '0.85rem', flexShrink: 0 }}>
                        {d.name?.charAt(0).toUpperCase()}
                    </div>
                    <div style={{ flex: 1, minWidth: 0 }}>
                        <p style={{ margin: 0, fontWeight: 700, color: C.navy, fontSize: '0.85rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{d.name}</p>
                        <p style={{ margin: '0.1rem 0 0', fontSize: '0.72rem', color: C.faint }}>{d.count} orders</p>
                    </div>
                    <div style={{ textAlign: 'right', flexShrink: 0 }}>
                        <p style={{ margin: 0, fontWeight: 800, color: C.navy, fontSize: '0.88rem' }}>{fmtSh(d.total)}</p>
                        <p style={{ margin: '0.1rem 0 0', fontSize: '0.7rem', color: C.faint }}>{share.toFixed(1)}%</p>
                    </div>
                </div>
                <div style={{ marginTop: '0.6rem', height: 5, background: '#f1f4f9', borderRadius: 999, overflow: 'hidden' }}>
    ...
    return (
        <div className="driver-row"
            ...
            <div style={{ width: 26, height: 26, borderRadius: 8, background: i < 3 ? `${medals[i]}15` : '#f1f4f9', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                {i < 3 ? <Award size={13} color={medals[i]} /> : <span style={{ fontSize: '0.7rem', fontWeight: 700, color: C.faint }}>{i + 1}</span>}
            </div>
...
const Revenue = () => {
    ...
    return (
        <div style={{ fontFamily: "'Sora', system-ui, sans-serif", padding: '0 1rem 3rem', maxWidth: 1200, margin: '0 auto', boxSizing: 'border-box' }}>
            <style>{`
                ...
                *, *::before, *::after { box-sizing: border-box; }
                ...
                .desk-driver-header { display:grid !important; }
                ...
                @media(max-width:640px)  {
                    ...
                    .desk-driver-header { display:none !important; }
                }
                @media(max-width:400px)  { .kpi-grid { grid-template-columns:1fr; } }
                .period-tab  { border:none; cursor:pointer; border-radius:9px; padding:0.4rem 0.7rem; font-size:0.75rem; font-weight:600; font-family:inherit; transition:all 0.15s; }
                .driver-row:hover { background:#f8fafc !important; }
            `}</style>

            {/* Header */}
            <div style={{ marginBottom: '1.5rem', marginTop: '0.5rem', animation: 'fadeInUp 0.35s ease both' }}>
            ...
            {loading ? (
                ...
                <>
                    ...
                    <div className="rev-grid" style={{ marginBottom: '1.5rem' }}>
                        ...
                        {/* Period selector */}
                        <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, padding: '1.25rem', ...shadow, animation: 'fadeInUp 0.4s ease 0.2s both' }}>
                            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '1.25rem', flexWrap: 'wrap', gap: '0.6rem' }}>
                                ...
                                <div style={{ display: 'flex', gap: 3, background: '#f1f4f9', borderRadius: 11, padding: 3 }}>
                                    {tabs.map(t => (
                            ...
                            <div style={{ textAlign: 'center', padding: '1.25rem 0' }}>
                                <p style={{ margin: 0, fontSize: isMobile ? '2.2rem' : '3rem', fontWeight: 900, color: C.navy, letterSpacing: '-1px', lineHeight: 1 }}>
                                    {fmt(current.total)}
                                </p>
                            ...
                            <div style={{ borderTop: `1px solid ${C.border}`, paddingTop: '1rem', display: 'flex', flexDirection: 'column', gap: '0.5rem' }}>
                                {tabs.map(t => (
                                    <div key={t.key} onClick={() => setPeriod(t.key)}
                                        style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0.55rem 0.75rem', borderRadius: 10, background: period === t.key ? `${C.navy}08` : 'transparent', border: `1px solid ${period === t.key ? `${C.navy}20` : 'transparent'}`, cursor: 'pointer', transition: 'all 0.15s' }}>
                        ...
                        {/* Bar chart */}
                        <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, padding: '1.25rem', ...shadow, animation: 'fadeInUp 0.4s ease 0.25s both' }}>
                    ...
                    <div style={{ background: C.white, borderRadius: 16, border: `1px solid ${C.border}`, overflow: 'hidden', ...shadow, animation: 'fadeInUp 0.4s ease 0.3s both' }}>
                        <div style={{ padding: '1.1rem 1.25rem', borderBottom: `1px solid ${C.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '0.65rem' }}>
                            <div>
                            ...
                            <div style={{ display: 'flex', gap: 6 }}>
                                {[{ key: 'total', label: 'By Revenue' }, { key: 'count', label: 'By Orders' }].map(s => (
                                    <button key={s.key} onClick={() => setDriverSort(s.key)}
                                        style={{ padding: '0.35rem 0.75rem', borderRadius: 8, border: `1px solid ${driverSort === s.key ? C.navy : C.border}`, background: driverSort === s.key ? C.navy : 'transparent', color: driverSort === s.key ? '#fff' : C.muted, fontSize: '0.73rem', fontWeight: 600, cursor: 'pointer', fontFamily: 'inherit', transition: 'all 0.15s' }}>
                                        {s.label}
                                    </button>
                                ))}
                            </div>
                        ...
                        {byDriver.length === 0 ? (
                            ...
                            <div>
                                ...
                                <div className="desk-driver-header" style={{ gridTemplateColumns: '2rem 1fr 120px 80px 160px', gap: '0.75rem', padding: '0.65rem 1.5rem', background: '#f8fafc', borderBottom: `1px solid ${C.border}` }}>
                                ...
                                {byDriver.map((d, i) => {
                                    ...
                                    return <DriverRow key={d.id} d={d} i={i} share={share} isMobile={isMobile} />;
                                })}
                            </div>
                        )}
                    </div>
