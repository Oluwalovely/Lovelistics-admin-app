...
const OrderDetails = () => {
    ...
    useEffect(() => {
        ...
        if (isActive) {
            const poll = async () => {
                try {
                    const res = await getLatestLocation(order._id);
                    ...
                    <div className="col-md-8">
                        ...
                        {['picked-up', 'in-transit'].includes(order.status) && (
                            <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14, borderLeft: '4px solid #198754' }}>
                                <div className="card-body">
                                    <h6 className="fw-bold mb-3 d-flex align-items-center gap-2" style={{ color: '#0d1f4f' }}>
                                        <Navigation size={16} /> Driver Live Location
                                    </h6>
                                    {driverLocation ? (
                                        <>
                                            <div className="row g-2 mb-2">
                                                <div className="col-6">
                                                    <p className="mb-1 text-muted small">Latitude</p>
                                                    <p className="mb-0 fw-semibold small">{driverLocation.location.lat.toFixed(6)}</p>
                                                </div>
                                                <div className="col-6">
                                                    <p className="mb-1 text-muted small">Longitude</p>
                                                    <p className="mb-0 fw-semibold small">{driverLocation.location.lng.toFixed(6)}</p>
                                                </div>
                                                {driverLocation.speed && (
                                                    <div className="col-6">
                                                        <p className="mb-1 text-muted small">Speed</p>
                                                        <p className="mb-0 fw-semibold small">{driverLocation.speed} m/s</p>
                                                    </div>
                                                )}
                                                <div className="col-6">
                                                    <p className="mb-1 text-muted small">Last Updated</p>
                                                    <p className="mb-0 fw-semibold small">{new Date(driverLocation.recordedAt).toLocaleTimeString()}</p>
                                                </div>
                                            </div>
                                            <a
                                                href={`https://www.google.com/maps?q=${driverLocation.location.lat},${driverLocation.location.lng}`}
                                                target="_blank"
                        ...
                        {!['delivered', 'confirmed', 'cancelled'].includes(order.status) && (
                            <div className="card border-0 shadow-sm mb-3" style={{ borderRadius: 14 }}>
                                <div className="card-body">
                                    <h6 className="fw-bold mb-3 d-flex align-items-center gap-2" style={{ color: '#0d1f4f' }}>
                                        <Car size={16} /> {order.driver ? 'Reassign Driver' : 'Assign Driver'}
                                    </h6>
                                        ...
                                        >
                                            ...
                                            {unavailableDrivers.map(d => (
                                                <option key={d._id} value={d._id} disabled>
                                                    {d.fullName} — {d.phone} (on delivery)
                                                </option>
                                            ))}
                                        </select>
                                        <button
                ...
                <div className="modal-dialog modal-dialog-centered">
                    <div className="modal-content border-0" style={{ borderRadius: 16 }}>
                        <div className="modal-body text-center p-4">
                            ...
                            <p className="text-muted small mb-4">
                                This will permanently remove the order from the system. This cannot be undone.
                            </p>
                            <div className="d-flex gap-2 justify-content-center">
                                <button
                                    className="btn btn-outline-secondary px-4"
